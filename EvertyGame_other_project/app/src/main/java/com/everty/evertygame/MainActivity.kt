        ...
        Box(
            ...
            Column(
                ...
                AdaptiveRoiSplitStreamCard(
                    ...
                    forcedOff = selectedTransport == StreamTransport.ADB_TUNNEL_TCP,
                    onEnabledChange = { enabled ->
                        ...
                        TransportSelector(
                            ...
                            onSelected = { transport ->
                                ...
                                if (transport == StreamTransport.ADB_TUNNEL_TCP) {
                                    host = "127.0.0.1"
                                    adaptiveRoiSplitStreamEnabled = false
                                    if (portText.isBlank()) {
                                        portText = "5001"
                                    }
                                } else if (host == "127.0.0.1") {
                                    host = "192.168.0.5"
                                    adaptiveRoiSplitStreamEnabled = true
                                }
                                inlineError = null
                            },
                        )
...
private fun buildConfigOrNull(
    ...
    return StreamConfig(
        ...
        touchLatencySprintEnabled = touchLatencySprintEnabled,
        gamepadBoostEnabled = gamepadBoostEnabled,
        adaptiveRoiSplitStreamEnabled = adaptiveRoiSplitStreamEnabled && transport == StreamTransport.UDP,
    )
}
        ...
        Column(
            ...
            Row(
                ...
                Column(
                    ...
                    Text(
                        text = if (enabled && !forcedOff) "Split-stream enabled" else "Split-stream disabled",
                        style = MaterialTheme.typography.bodyLarge,
                        fontWeight = FontWeight.Medium,
                    )
                    Text(
                        text = if (forcedOff) {
                            "ADB tunnel / TCP uses base stream only"
                        } else {
                            "UDP / LAN can send base + ROI enhancement"
                        },
                        style = MaterialTheme.typography.bodySmall,
                        color = if (forcedOff) MaterialTheme.colorScheme.onSurfaceVariant else Color(0xFF25C77A),
                    )
                }
                Switch(
