...
if ([string]::IsNullOrWhiteSpace($OutDir)) {
    $OutDir = Join-Path $root "reports\evrtck-prod\$timestamp"
}
...
try {
    ...
    $metadata = [ordered]@{
        ...
        profile = "release"
        command = "cargo run --release --bin evrtck_prod_bench"
        commit = $commit.Trim()
        branch = $branch.Trim()
        ...
        cargo = $cargo
        outputs = [ordered]@{
            csv = (Join-Path $OutDir "evrtck_prod_bench.csv")
            jsonl = (Join-Path $OutDir "evrtck_prod_bench.jsonl")
    ...
    cargo @args
    if ($LASTEXITCODE -ne 0) {
        throw "EVRTCK production bench failed with exit code $LASTEXITCODE"
    }

    $summary = & (Join-Path $PSScriptRoot "summarize-evrtck-prod-bench.ps1") -ReportDir $OutDir
