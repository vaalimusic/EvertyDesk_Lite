...
impl DecodedImage {
    ...
    pub(crate) fn apply_rgb16_bitmap(
        ...
        // `rect_fits` above only validates the *declared* rectangle against
        // the image bounds — it says nothing about whether `rgb16` actually
        // contains exactly `rectangle_width * rectangle_height` pixels. If
        // the server sends more row data than the rectangle declares (seen
        // in practice against VirtualBox VRDE — a malformed/inconsistent
        // bitmap update, not something a spec-compliant server should ever
        // do), `chunks_exact` happily yields extra rows and `row_idx` walks
        // straight past the validated bound, indexing `self.data` out of
        // range and panicking. Capping at `rectangle_height` makes the
        // already-intended bound actually hold.
        rgb16
    ...
    pub(crate) fn apply_rgb15_bitmap(
        ...
        // See the matching comment in apply_rgb16_bitmap: caps row_idx at the
        // already-validated rectangle bound regardless of how much source
    ...
    pub(crate) fn apply_bgr24_bitmap(
        ...
        // See the matching comment in apply_rgb16_bitmap — this is the exact
        // path that paniced against VirtualBox VRDE in practice ("index out
        // of bounds: the len is N but the index is N+k"): a BGR24 update
        // whose source data had more rows than its own declared rectangle.
        bgr24
            .chunks_exact(rectangle_width * SRC_COLOR_DEPTH)
            .rev()
            .take(rectangle_height)
    ...
    pub(crate) fn apply_rgb8_with_palette(
        ...
        // See the matching comment in apply_rgb16_bitmap.
        indexed
        ...
        I: Iterator<Item = &'a [u8]>,
    {
        ...
        // See the matching comment in apply_rgb16_bitmap — fixed here in the
        // generic-iterator form so it covers `apply_rgb24` regardless of how
        // the caller constructed `rgb24`.
        rgb24.take(rectangle_height).enumerate().for_each(|(row_idx, row)| {
            row.chunks_exact(SRC_COLOR_DEPTH)
                .enumerate()
                .for_each(|(col_idx, src_pixel)| {
                    let dst_idx = ((top + row_idx) * image_width + left + col_idx) * DST_COLOR_DEPTH;
        ...
        I: Iterator<Item = &'a [u8]>,
    {
        ...
        // See the matching comment in apply_rgb16_bitmap. The QOI caller
        // already validates `decoded.len()` against the expected size, so
        // this is belt-and-suspenders rather than fixing a reachable bug,
        // but the cost of being consistent here is zero.
        rgba32.take(rectangle_height).enumerate().for_each(|(row_idx, row)| {
            row.chunks_exact(SRC_COLOR_DEPTH)
                .enumerate()
                .for_each(|(col_idx, src_pixel)| {
                    let dst_idx = ((top + row_idx) * image_width + left + col_idx) * DST_COLOR_DEPTH;
    ...
    pub(crate) fn apply_rgb32_bitmap(
        ...
        // See the matching comment in apply_rgb16_bitmap. 32bpp is the depth
        // this app actually negotiates, so this branch (and the one below)
        // is the most likely path for the same VirtualBox VRDE
        // oversized-update quirk that crashed apply_bgr24_bitmap in practice.
