...
pub(crate) struct MppcContext {
    ...
    /// Decompression output staging buffer.
    ///
    /// `history_ptr` is treated as an ever-growing *logical* position; all
    /// reads/writes against `history_buffer` go through `& history_mask` to
    /// get the physical (ring) offset. That means the bytes decoded for a
    /// single `decompress()` call can legitimately wrap around the physical
    /// buffer boundary, which can't be expressed as one contiguous slice of
    /// `history_buffer`. This scratch buffer decouples "what we return to the
    /// caller" from "the ring used for back-references", so the ring never
    /// needs an artificial reset/overflow-error path: it just wraps, exactly
    /// like the `src_start` computation for CopyOffset already does.
...
impl MppcContext {
    ...
    pub(crate) fn decompress<'a>(&'a mut self, src_data: &'a [u8], flags_value: u32) -> Result<&'a [u8], BulkError> {
        ...
        // `history_ptr` is treated as a logical, ever-growing position and
        // is only ever reset by an explicit AT_FRONT/FLUSHED signal above —
        // exactly per spec. All physical addressing into `history_buffer`
        // goes through `& history_mask`, so the ring wraps naturally instead
        // of needing a bounds check at all.
        //
        // An earlier version of this patch reset `history_ptr` unilaterally
        // whenever it got close to (or exceeded) the buffer end, to work
        // around VirtualBox VRDE not reliably sending AT_FRONT/FLUSHED
        // before its own encoder history would wrap. That was wrong:
        // `history_ptr` is shared protocol state between the server's
        // encoder and this decoder. Resetting only our side desyncs us from
        // the server's still-unreset encoder position — every subsequent
        // CopyOffset is encoded relative to a server-side position we no
        // longer agree on, which decoded as garbage/black for as long as
        // the mismatch persisted. Letting the *physical* address wrap via
        // the mask (like `src_start` already did for CopyOffset sources)
        // while the *logical* position keeps counting up avoids the need
        // for any unilateral reset, and matches what a real ring-buffer
        // implementation (e.g. FreeRDP's) does.
        ...
        while bs.remaining_bits() >= 8 {
            ...
            if (accumulator & 0x8000_0000) == 0x0000_0000 {
                ...
                self.history_buffer[history_ptr & history_mask] = literal;
                self.output_scratch.push(literal);
                history_ptr += 1;
                bs.shift(8);
                continue;
            ...
            // Copy from history buffer at (current - copy_offset), reading and
            // writing byte-by-byte: both source and destination physical
            // addresses wrap independently via `& history_mask`, which also
            // correctly handles the LZ77 self-overlapping-repeat case (e.g.
            // copy_offset=1 to run-length-extend the last byte) since each
            // byte is written before the next one is read.
            let mut src_index = history_ptr.wrapping_sub(copy_offset) & history_mask;
            for _ in 0..length_of_match {
                let byte = self.history_buffer[src_index];
                self.history_buffer[history_ptr & history_mask] = byte;
                self.output_scratch.push(byte);
                history_ptr += 1;
