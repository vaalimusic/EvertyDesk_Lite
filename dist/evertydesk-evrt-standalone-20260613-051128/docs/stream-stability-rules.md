...
- The default desktop connection path is relay-first. Direct probes are useful
  for diagnostics and future EVRT work, but they must not delay a normal support
  session.
- Do not open a relay TCP socket as a preflight if the session will open a real
  relay stream later.
- Blocking UDP NAT detection is opt-in with `EVERTYDESK_CONNECT_UDP_PROBE=1`.
- Direct TCP probing is opt-in with `EVERTYDESK_CONNECT_RELAY_FIRST=0` and
  `EVERTYDESK_TRY_DIRECT_TCP=1`.
- After display subscription, request one screenshot immediately while live
  video negotiates codecs and waits for its first keyframe.
- Connection progress should include stage timings for ID server, rendezvous,
  relay stream open, and peer login/approval wait.
