...
6. EVRTCK production evidence.
   ...
   - Done: hardware decode and encode+decode roundtrip benchmarking run in crash-isolated child processes with GOP-oriented decoder state; H.264 produced valid local rows, and local HEVC decoder access violations are captured as CSV/JSONL errors instead of killing the parent benchmark.
   - Still required: full publishable non-quick hardware decode/roundtrip run on a machine where HEVC decode is stable, then feed those rows into the EVRTCK-vs-hardware decision report.
