...
6. EVRTCK production evidence.
   ...
   - Done: hardware decode and encode+decode roundtrip benchmarking run in crash-isolated child processes with GOP-oriented decoder state; tiny hardware packets are rejected instead of being published as real frame latency, and local HEVC decoder access violations are captured as CSV/JSONL errors instead of killing the parent benchmark.
   - Still required: full publishable non-quick hardware decode/roundtrip run with meaningful H.264/H.265 packet payloads, then feed those rows into the EVRTCK-vs-hardware decision report.
