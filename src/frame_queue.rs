...
impl FrameQueue {
    ...
    fn enqueue_inner(g: &mut Inner, bytes: Vec<u8>, is_key_frame: bool, presentation_time_us: u64) {
        if g.closed {
            return;
        }
        ...
        // ── hard reset на IDR ─────────────────────────────────────────────────
        if g.cfg.hard_reset_on_keyframe
            && is_key_frame
    ...
    pub fn dequeue(&self, cancel: &std::sync::atomic::AtomicBool) -> Option<(Vec<u8>, bool, u64)> {
...
impl AdaptiveRelief {
    ...
    pub fn on_feedback(
        ...
        if strained {
            ...
            if pressure_critical {
                w += 3;
            } else if pressure_high {
                w += 2;
            }
            if decode_behind {
                w += if decode_collapsed { 3 } else { 2 };
            }
            if present_elevated {
                w += if present_high { 2 } else { 1 };
            }
    ...
    pub fn apply_pending(&mut self) -> Option<f32> {
        self.apply_pending_milli()
            .map(|scale_milli| scale_milli as f32 / 1_000.0)
    }

    pub fn apply_pending_milli(&mut self) -> Option<u32> {
        let step = self.pending_step.take()?;
        self.step = step;
        Some(Self::bitrate_scale_milli(step))
    }
    ...
    pub fn bitrate_scale(step: u8) -> f32 {
        Self::bitrate_scale_milli(step) as f32 / 1_000.0
    }

    pub fn bitrate_scale_milli(step: u8) -> u32 {
        match step {
            0 => 1_000,
            1 => 880, // High quality: -12%
            _ => 800, // Low  quality: -20%
        }
    }
}
...
mod tests {
    ...
    fn reassembler_multi_packet_frame() {
        ...
        for (i, raw) in packets.iter().enumerate() {
            ...
            if i < last_idx {
                assert!(
                    result.is_none(),
                    "не должен быть готов до последнего пакета"
                );
            } else {
                assert!(
                    result.is_some(),
                    "должен быть готов после последнего пакета"
                );
            }
        }
    }
    ...
    #[test]
    fn adaptive_relief_apply_pending_updates_bitrate_scale() {
        use crate::evrt::{Pressure, ReceiverFeedback};
        let mut relief = AdaptiveRelief::new(true);
        let fb = ReceiverFeedback {
            pressure: Pressure::Critical,
        ...
        assert_eq!(relief.on_feedback(&fb, 60), Some(1));
        assert_eq!(relief.apply_pending_milli(), Some(880));
        assert_eq!(relief.current_step(), 1);
        assert_eq!(AdaptiveRelief::bitrate_scale_milli(2), 800);
    }
