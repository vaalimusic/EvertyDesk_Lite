...
impl Default for DisplayConfig {
    fn default() -> Self {
        Self {
            // Обычный режим = EVRTCK явно (не «Авто»): пытается прямой EVRT
            // (LAN/VPN, с WiFi) и всегда падает на TCP relay с авто-кодеком
            // (без WiFi/через интернет) — работает универсально в обоих случаях.
            codec: CodecPreference::Evrtck,
...
pub struct UiConfig {
    ...
    #[serde(default)]
    pub ui_scale: f32,
    /// Показывать техническую информацию хоста (fps/битрейт/энкодер) во время
    /// сессии. По умолчанию выключено — не мешает обычному пользователю;
    /// включается в Настройках для диагностики.
    #[serde(default)]
    pub show_tech_info: bool,
}
...
pub struct ContactEntry {
    ...
    #[serde(default)]
    pub online: bool,
    /// Address-book labels. In multi-AB API this maps to Peer.tags.
    #[serde(default)]
    pub tags: Vec<String>,
}
...
pub struct AppConfig {
    ...
    /// Runtime-only unattended password loaded from the OS credential store.
    /// It is intentionally not persisted to `config.json`.
    ...
    pub fn load_or_create() -> Self {
        ...
        if let Ok(raw) = fs::read_to_string(&path) {
            if let Ok(mut config) = serde_json::from_str::<Self>(&raw) {
                ...
                // Migrate letter-prefixed IDs (e.g. "g84357127") generated by a
                // broken build — replace with a fresh 9-digit numeric ID.
                if config
                    .local_id
                    .chars()
                    .next()
                    ...
                    .unwrap_or(false)
                {
                    config.local_id = generate_numeric_token(9);
                    changed = true;
                }
                ...
                // Существующие конфиги могли сохранить codec=Auto (старый
                // дефолт). Обычный режим должен быть явно EVRTCK — мигрируем,
                // иначе TCP-фоллбэк для Auto продолжит свободно выбирать
                // аппаратный H265, а на некоторых системах системный HEVC-
                // декодер Windows (HEVCDECODER_STORE.dll) падает с access
                // violation на этом пути (см. preferred_codec в transport.rs).
                if config.display.codec == CodecPreference::Auto {
                    config.display.codec = CodecPreference::Evrtck;
                    changed = true;
                }
                // AI Hotfix выключен по умолчанию. Если он включён, но API-ключ не
                // задан — включённый флаг всё равно ничего не делает (submit_crash
                // пропускается при пустом ключе), поэтому гасим его, чтобы новые
                // установки не показывали фичу активной. Пользователь, реально
                // настроивший ключ, сохранит включённое состояние.
                if config.hotfix.enabled && config.hotfix.api_key.trim().is_empty() {
                    config.hotfix.enabled = false;
                    changed = true;
                }
