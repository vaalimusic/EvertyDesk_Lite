...
impl EvertyDeskApp {
    ...
    #[allow(dead_code)]
    fn contacts_ui_legacy(&mut self, ui: &mut egui::Ui) {
        ui.label(
        ...
        if self.config.ui.address_book_signed_in && self.config.ui.contacts.is_empty() {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    let columns: usize = if available >= 920.0 {
                        4
                    ...
                    for row in visible_indices.chunks(columns) {
                        ui.horizontal(|ui| {
                            ui.spacing_mut().item_spacing.x = gap;
                            for &idx in row {
                                let contact = self.config.ui.contacts[idx].clone();
                                ui.allocate_ui_with_layout(
                                    egui::vec2(tile_width, 138.0),
                                    egui::Layout::top_down(egui::Align::Min),
                                    |ui| {
                                        contact_tile_frame(contact.online).show(ui, |ui| {
                                            ui.set_min_size(egui::vec2(
                                                (tile_width - 22.0).max(120.0),
                                                116.0,
                                            ));
                                            ui.horizontal(|ui| {
                                                ui.vertical(|ui| {
                                                    ui.set_max_width((tile_width - 72.0).max(88.0));
                                                    ui.add_sized(
                                                    ...
                                                    ui.add_sized(
                                                        ...
                                                        egui::Label::new(
                                                            egui::RichText::new(format_peer_id(
                                                                &contact.remote_id,
                                                            ))
                                                            .size(12.5)
                                                            .monospace()
                                                ...
                                                ui.with_layout(
                                                    ...
                                                    |ui| {
                                                        ...
                                                        menu.response.on_hover_text(tr(
                                                            lang,
                                                            "Действия",
                                                            "Actions",
                                                        ));
                                            ...
                                            ui.add_sized(
                                                egui::vec2(ui.available_width(), 18.0),
                                                egui::Label::new(
                                                    egui::RichText::new(note).size(12.0).color(
                                                        egui::Color32::from_rgb(0x67, 0x70, 0x80),
                                                    ),
                                                )
                                                .wrap(false),
                                            );
