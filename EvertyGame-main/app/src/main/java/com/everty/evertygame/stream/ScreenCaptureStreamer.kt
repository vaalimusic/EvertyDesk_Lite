...
class ScreenCaptureStreamer(
    ...
    private val captureThread = HandlerThread("EvertyCaptureRender")
    ...
    private var sessionSplitStreamEnabled: Boolean? = null
    ...
    fun stop() {
        ...
        if (captureThread.isAlive) {
            runCatching { captureHandler.removeCallbacksAndMessages(null) }
        }
        ...
        virtualDisplay = null
        splitStreamHub = null
        sessionSplitStreamEnabled = null
        audioStreamer = null
    ...
    private fun startStreamingOnWorker() {
        try {
            ...
            if (
                config.audioEnabled &&
                config.adaptationMode != AdaptationMode.LATENCY_ATTACK &&
    ...
    private fun maybeReconfigureVideoCapture() {
        ...
        val captureDeltaPx = maxOf(
            kotlin.math.abs(captureSize.width - activeWidth),
            kotlin.math.abs(captureSize.height - activeHeight),
        )
        val captureChangedSignificantly =
            activeWidth <= 0 ||
                activeHeight <= 0 ||
                orientationChanged ||
                captureDeltaPx >= 96
        if (!displayChanged && !captureChangedSignificantly) {
            lastDisplayRotation = displayInfo.rotation
            lastDisplayWidth = displayInfo.width
            lastDisplayHeight = displayInfo.height
            lastDisplayDensityDpi = displayInfo.densityDpi
    ...
    private fun attachConfiguredVideoPipeline(
        ...
        splitStreamEnabled = sessionSplitStreamEnabled ?: shouldUseSplitStream().also {
            sessionSplitStreamEnabled = it
        }
        ...
        val displaySurface = if (splitStreamEnabled) {
            val configuredEnhancementEncoder = if (splitStreamEnabled) {
                configureEnhancementEncoder(captureSize, displayInfo)
            } else {
                null
            }
            enhancementEncoder = configuredEnhancementEncoder?.encoder
            enhancementSurfaceSize = configuredEnhancementEncoder?.width ?: 0
            splitStreamHub = SplitStreamGlHub(
                callbackHandler = captureHandler,
                ...
                enhancementSurfaceWidth = configuredEnhancementEncoder?.width ?: 0,
                enhancementSurfaceHeight = configuredEnhancementEncoder?.height ?: 0,
                ...
                roiProvider = if (splitStreamEnabled) {
                    {
                ...
                onEnhancementRendered = if (splitStreamEnabled) {
                    { presentationTimeUs, roiSnapshot ->
    ...
    private fun updateBitrateTargets(configuredEncoder: ConfiguredEncoder) {
        ...
        minBitrateBps = when (config.preset) {
            ...
            -> when (config.adaptationMode) {
                ...
                AdaptationMode.CINEMA_SMOOTH -> (configuredEncoder.bitrateBps * 0.76f).toInt().coerceAtLeast(1_900_000)
                AdaptationMode.WI_FI_LAN_TURBO -> (configuredEncoder.bitrateBps * 0.84f).toInt().coerceAtLeast(2_600_000)
                AdaptationMode.LATENCY_ATTACK -> (configuredEncoder.bitrateBps * 0.18f).toInt().coerceAtLeast(450_000)
            ...
            QualityPreset.INSTANT_PLAY -> when (config.adaptationMode) {
                ...
                AdaptationMode.CINEMA_SMOOTH -> (configuredEncoder.bitrateBps * 0.72f).toInt().coerceAtLeast(1_200_000)
                AdaptationMode.WI_FI_LAN_TURBO -> (configuredEncoder.bitrateBps * 0.80f).toInt().coerceAtLeast(1_600_000)
                AdaptationMode.LATENCY_ATTACK -> (configuredEncoder.bitrateBps * 0.16f).toInt().coerceAtLeast(400_000)
                ...
                AdaptationMode.CINEMA_SMOOTH -> (configuredEncoder.bitrateBps * 0.74f).toInt().coerceAtLeast(1_400_000)
                AdaptationMode.WI_FI_LAN_TURBO -> (configuredEncoder.bitrateBps * 0.78f).toInt().coerceAtLeast(2_000_000)
                AdaptationMode.LATENCY_ATTACK -> (configuredEncoder.bitrateBps * 0.15f).toInt().coerceAtLeast(400_000)
        ...
        criticalMinBitrateBps = when (config.preset) {
            ...
            -> when (config.adaptationMode) {
                ...
                AdaptationMode.CINEMA_SMOOTH -> (configuredEncoder.bitrateBps * 0.56f).toInt().coerceAtLeast(1_300_000)
                AdaptationMode.WI_FI_LAN_TURBO -> (configuredEncoder.bitrateBps * 0.62f).toInt().coerceAtLeast(2_000_000)
                AdaptationMode.LATENCY_ATTACK -> (configuredEncoder.bitrateBps * 0.12f).toInt().coerceAtLeast(350_000)
            ...
            QualityPreset.INSTANT_PLAY -> when (config.adaptationMode) {
                ...
                AdaptationMode.CINEMA_SMOOTH -> (configuredEncoder.bitrateBps * 0.48f).toInt().coerceAtLeast(700_000)
                AdaptationMode.WI_FI_LAN_TURBO -> (configuredEncoder.bitrateBps * 0.55f).toInt().coerceAtLeast(900_000)
                AdaptationMode.LATENCY_ATTACK -> (configuredEncoder.bitrateBps * 0.10f).toInt().coerceAtLeast(300_000)
                ...
                AdaptationMode.CINEMA_SMOOTH -> (configuredEncoder.bitrateBps * 0.50f).toInt().coerceAtLeast(900_000)
        ...
        if (config.transport == StreamTransport.ADB_TUNNEL_TCP && config.adaptationMode != AdaptationMode.LATENCY_ATTACK) {
    ...
    private fun configureEncoder(
        ...
        for (attempt in buildEncoderAttempts(width, height, preset)) {
            val encoder = createVideoEncoder(channel)
            try {
                ...
                if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK && Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    runCatching {
    ...
    private fun createVideoEncoder(channel: VideoChannel): MediaCodec {
        if (channel != VideoChannel.BASE || config.adaptationMode != AdaptationMode.LATENCY_ATTACK) {
            return MediaCodec.createEncoderByType(config.codec.mimeType)
        }
            ...
            MediaCodecList(MediaCodecList.REGULAR_CODECS).codecInfos
                ...
                .filter { codecInfo ->
                    codecInfo.supportedTypes.any { type ->
                        type.equals(config.codec.mimeType, ignoreCase = true)
                    }
                }
                .mapNotNull { codecInfo ->
                    val capabilities = runCatching {
                        codecInfo.getCapabilitiesForType(config.codec.mimeType)
                    }.getOrNull() ?: return@mapNotNull null
                    val supportsSurfaceInput = capabilities.colorFormats.any { colorFormat ->
                        colorFormat == MediaCodecInfo.CodecCapabilities.COLOR_FormatSurface
                    }
                    if (!supportsSurfaceInput) {
                    ...
                    if ("qcom" in name || "qti" in name || "mtk" in name || "exynos" in name) {
                        score += 40
                    }
                    if ("android" in name || "google" in name || "sw" in name) {
                        score -= 120
                    }

                    score to codecInfo.name
                }
        ...
        return if (codecName.isNullOrBlank()) {
    ...
    private fun buildEncoderAttempts(
        ...
        val requestedFps = config.targetFps.coerceIn(24, 120)
        val targetFps = if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK) {
            requestedFps.coerceAtMost(20)
        } else {
            requestedFps
        }
        val targetBitrateBps = if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK) {
            val ceiling = if (config.transport == StreamTransport.UDP) 1_600_000 else 2_400_000
            config.targetBitrateBps.coerceIn(700_000, ceiling)
        } else {
            config.targetBitrateBps.coerceIn(1_000_000, 100_000_000)
        }
        ...
        val transportBitrateFactor = when (config.transport) {
            StreamTransport.UDP -> when (preset) {
                QualityPreset.TOURNAMENT_FIGHTER -> when (config.adaptationMode) {
                    AdaptationMode.WI_FI_LAN_TURBO -> 1.55f
                    AdaptationMode.CINEMA_SMOOTH -> 1.08f
                    AdaptationMode.LATENCY_ATTACK -> 0.54f
                    else -> 0.92f
                }
                QualityPreset.WI_FI_GAMING -> when (config.adaptationMode) {
                    AdaptationMode.WI_FI_LAN_TURBO -> 1.25f
                    AdaptationMode.CINEMA_SMOOTH -> 1.10f
                    AdaptationMode.LATENCY_ATTACK -> 0.58f
                    else -> 1.0f
                }
                QualityPreset.COMPETITIVE -> when (config.adaptationMode) {
                    AdaptationMode.WI_FI_LAN_TURBO -> 1.45f
                    AdaptationMode.CINEMA_SMOOTH -> 1.06f
                    AdaptationMode.LATENCY_ATTACK -> 0.56f
                    else -> 0.96f
                }
                QualityPreset.BALANCED_LOW_LATENCY -> when (config.adaptationMode) {
                    AdaptationMode.WI_FI_LAN_TURBO -> 1.30f
                    AdaptationMode.CINEMA_SMOOTH -> 1.08f
                    AdaptationMode.LATENCY_ATTACK -> 0.56f
                    else -> 1.0f
                }
                QualityPreset.INSTANT_PLAY -> when (config.adaptationMode) {
                    AdaptationMode.WI_FI_LAN_TURBO -> 1.15f
                    AdaptationMode.CINEMA_SMOOTH -> 1.02f
                    AdaptationMode.LATENCY_ATTACK -> 0.64f
                    else -> 1.0f
                }
                else -> 1.0f
            }
        ...
        val effectivePresetBitrate = (targetBitrateBps * areaScale * transportBitrateFactor)
            .roundToInt()
            .coerceAtLeast(if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK) 500_000 else 1_400_000)
        val operatingRateMultiplier = when (config.transport) {
            ...
            if (config.adaptationMode == AdaptationMode.CINEMA_SMOOTH && config.transport == StreamTransport.UDP) 0.08f else 0f +
            if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK && config.transport == StreamTransport.UDP) 0.18f else 0f +
        ...
        fpsCandidates.forEachIndexed { index, fps ->
            val bitrate = ((effectivePresetBitrate.toLong() * fps) / targetFps)
                .toInt()
                .coerceAtLeast(if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK) 450_000 else 1_200_000)
            val operatingRateFps = if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK) {
                fps.toFloat()
            } else {
                maxOf(fps.toFloat() * operatingRateMultiplier, fps + 6f)
            }
            val keyFrameIntervalSeconds = if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK) 2 else preset.keyFrameIntervalSeconds

            if (config.codec == VideoCodec.AVC) {
                if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK && index == 0) {
    ...
    private fun sendSessionConfigPacket(
        ...
        val payload = buildString {
            ...
            append("\"adaptationMode\":\"${config.adaptationMode.name}\",")
    ...
    private fun sendVideoFrame(
        ...
        val senderPipelineMs = ((System.nanoTime() / 1_000 - presentationTimeUs).coerceAtLeast(0L) / 1_000L).toInt()
        if (shouldDropLateEncodedFrame(senderPipelineMs = senderPipelineMs, isKeyFrame = isKeyFrame)) {
            totalDroppedFrames += 1
            emitMetricsIfNeeded()
            return
        }
        ...
        maybeSendLatencyLabPulseMarker(
            presentationTimeUs = presentationTimeUs,
        ...
        lastPipelineLatencyMs = senderPipelineMs
        emitMetricsIfNeeded()
    }
    ...
    private fun shouldDropLateEncodedFrame(senderPipelineMs: Int, isKeyFrame: Boolean): Boolean {
        if (config.adaptationMode != AdaptationMode.LATENCY_ATTACK || isKeyFrame) {
            return false
        }

        val maxSenderPipelineMs = when (config.transport) {
            StreamTransport.UDP -> 110
            StreamTransport.ADB_TUNNEL_TCP -> 140
        }
        return senderPipelineMs >= maxSenderPipelineMs
    }

    private fun maybeSendLatencyLabPulseMarker(
        presentationTimeUs: Long,
        senderPipelineMs: Int,
    ) {
        val marker = LatencyLabController.consumeMarkerForFrame(
        ...
        val payload = buildString {
            append("{")
            append("\"kind\":\"latency_pulse\",")
            append("\"pulseId\":${marker.pulseId},")
            append("\"source\":\"${marker.source}\",")
            append("\"presentationTimeUs\":${marker.presentationTimeUs},")
            append("\"tapToUiMs\":${marker.tapToUiMs},")
            append("\"senderPipelineMs\":${marker.senderPipelineMs},")
            append("\"approxSenderMs\":${marker.approxSenderMs}")
            append("}")
        }.toByteArray(Charsets.UTF_8)

        packetSender?.send(packetizer.buildControlPacket(payload))
        totalPacketsSent += 1
    }
    ...
    private fun shouldDropTransportFrame(frameId: Int, packetCount: Int, isKeyFrame: Boolean): Boolean {
        ...
        val now = SystemClock.elapsedRealtime()
        if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK) {
            return false
        }
    ...
    private fun maybeApplyExternalTouchLatencySprint(now: Long) {
        ...
        if (currentBitrateBps > minBitrateBps && now - lastAdaptiveActionAtMs >= 120L) {
            val sprintTrimFactor = when (config.adaptationMode) {
                ...
                AdaptationMode.CINEMA_SMOOTH -> when (pulseLevel) {
                    3 -> 0.94f
                    2 -> 0.96f
                    else -> 0.98f
                }
                ...
                AdaptationMode.LATENCY_ATTACK -> when (pulseLevel) {
                    3 -> 0.72f
                    2 -> 0.78f
                    else -> 0.84f
                }
    ...
    private fun handleControlPacket(payload: ByteArray) {
        when (val message = ControlMessage.parse(payload)) {
            ControlMessage.RequestKeyFrame -> {
                if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK) {
                    codecHandler.post {
                        requestSyncFrame(minIntervalMs = 1_500L)
                    }
                } else {
        ...
        when (message.pressure) {
                ...
                if (now - lastAdaptiveActionAtMs >= 180) {
                    val reductionFactor = when (config.preset) {
                        ...
                        -> when (config.adaptationMode) {
                            ...
                            AdaptationMode.CINEMA_SMOOTH -> 0.82f
                            AdaptationMode.WI_FI_LAN_TURBO -> 0.76f
                            AdaptationMode.LATENCY_ATTACK -> 0.38f
                        ...
                        QualityPreset.INSTANT_PLAY -> when (config.adaptationMode) {
                            ...
                            AdaptationMode.CINEMA_SMOOTH -> 0.78f
                            AdaptationMode.WI_FI_LAN_TURBO -> 0.72f
                            AdaptationMode.LATENCY_ATTACK -> 0.34f
                            ...
                            AdaptationMode.CINEMA_SMOOTH -> 0.76f
                            AdaptationMode.WI_FI_LAN_TURBO -> 0.70f
                            AdaptationMode.LATENCY_ATTACK -> 0.36f
                ...
                if (now - lastAdaptiveActionAtMs >= 350) {
                    val reductionFactor = when (config.preset) {
                        ...
                        -> when (config.adaptationMode) {
                            ...
                            AdaptationMode.CINEMA_SMOOTH -> 0.93f
                            AdaptationMode.WI_FI_LAN_TURBO -> 0.93f
                            AdaptationMode.LATENCY_ATTACK -> 0.62f
                        ...
                        QualityPreset.INSTANT_PLAY -> when (config.adaptationMode) {
                            ...
                            AdaptationMode.CINEMA_SMOOTH -> 0.90f
                            ...
                            AdaptationMode.CINEMA_SMOOTH -> 0.91f
                            AdaptationMode.WI_FI_LAN_TURBO -> 0.86f
                            AdaptationMode.LATENCY_ATTACK -> 0.60f
            ...
            ControlMessage.PressureLevel.NORMAL -> {
                ...
                val restoreDelayMs = when (config.adaptationMode) {
                    ...
                    AdaptationMode.CINEMA_SMOOTH -> 900L
                    AdaptationMode.WI_FI_LAN_TURBO -> 750L
                    AdaptationMode.LATENCY_ATTACK -> 8_000L
                ...
                if (!activeMotion &&
                    ...
                    val increaseFactor = when (config.adaptationMode) {
                        ...
                        AdaptationMode.CINEMA_SMOOTH -> 1.14f
                        AdaptationMode.WI_FI_LAN_TURBO -> 1.20f
                        AdaptationMode.LATENCY_ATTACK -> 1.02f
                ...
                if (canTrimForCadence) {
                    val trimFactor = when {
                        gamepadBoostActive && activeMotion && config.adaptationMode == AdaptationMode.CINEMA_SMOOTH -> 0.98f
                        gamepadBoostActive && activeMotion && config.adaptationMode == AdaptationMode.WI_FI_LAN_TURBO -> 0.95f
                        gamepadBoostActive && activeMotion && config.adaptationMode == AdaptationMode.LATENCY_ATTACK -> 0.72f
                        ...
                        gamepadBoostActive && config.adaptationMode == AdaptationMode.CINEMA_SMOOTH -> 0.985f
                        gamepadBoostActive && config.adaptationMode == AdaptationMode.WI_FI_LAN_TURBO -> 0.96f
                        gamepadBoostActive && config.adaptationMode == AdaptationMode.LATENCY_ATTACK -> 0.76f
                        gamepadBoostActive && config.adaptationMode == AdaptationMode.LOWEST_LATENCY -> 0.88f
                        gamepadBoostActive -> 0.92f
                        activeMotion && config.adaptationMode == AdaptationMode.CINEMA_SMOOTH -> 0.985f
                        activeMotion && config.adaptationMode == AdaptationMode.WI_FI_LAN_TURBO -> 0.96f
                        activeMotion && config.adaptationMode == AdaptationMode.LATENCY_ATTACK -> 0.80f
                        ...
                        activeMotion -> 0.94f
                        config.adaptationMode == AdaptationMode.CINEMA_SMOOTH -> 0.99f
                        config.adaptationMode == AdaptationMode.WI_FI_LAN_TURBO -> 0.98f
                        config.adaptationMode == AdaptationMode.LATENCY_ATTACK -> 0.84f
    ...
    private fun nextPeriodicSyncIntervalMs(now: Long): Long {
        ...
        return when (config.transport) {
            StreamTransport.ADB_TUNNEL_TCP -> if (config.adaptationMode == AdaptationMode.CINEMA_SMOOTH) 1_600L else 1_000L
            StreamTransport.UDP -> when (config.preset) {
                QualityPreset.WI_FI_GAMING,
                QualityPreset.TOURNAMENT_FIGHTER,
                QualityPreset.COMPETITIVE,
                -> if (config.adaptationMode == AdaptationMode.CINEMA_SMOOTH) 900L else 0L

                QualityPreset.BALANCED_LOW_LATENCY,
                -> if (config.adaptationMode == AdaptationMode.CINEMA_SMOOTH) 1_000L else 0L

                QualityPreset.INSTANT_PLAY -> if (config.adaptationMode == AdaptationMode.CINEMA_SMOOTH) 1_200L else 1_500L
                else -> if (config.adaptationMode == AdaptationMode.CINEMA_SMOOTH) 1_000L else 1_500L
            }
        }
    }
    ...
    private fun shouldUseSplitStream(): Boolean {
        return config.codec == VideoCodec.AVC &&
            config.adaptationMode != AdaptationMode.CINEMA_SMOOTH &&
            config.adaptationMode != AdaptationMode.LATENCY_ATTACK &&
    ...
    private fun capturePixelBudgetFactor(preset: QualityPreset): Float {
        if (config.adaptationMode == AdaptationMode.LATENCY_ATTACK) {
            return when (config.transport) {
                StreamTransport.UDP -> 0.18f
                StreamTransport.ADB_TUNNEL_TCP -> 0.24f
            }
        }
