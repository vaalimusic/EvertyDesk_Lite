...
internal class SplitStreamGlHub(
    ...
    private val enhancementInputSurface: Surface? = null,
    private val roiProvider: (() -> TouchLatencySprintController.RoiSnapshot?)? = null,
    private val onEnhancementRendered: ((Long, TouchLatencySprintController.RoiSnapshot) -> Unit)? = null,
    private val targetBaseFrameIntervalNs: Long = 0L,
) : SurfaceTexture.OnFrameAvailableListener, AutoCloseable {
    private val frameSignalSync = Any()
    ...
    private var lastBaseRenderTimestampNs = 0L

    init {
        runOnCallbackThreadBlocking { initializeGl() }
    }
    ...
    override fun onFrameAvailable(surfaceTexture: SurfaceTexture?) {
        ...
        synchronized(frameSignalSync) {
            pendingFrameSignals += 1
            if (!drainScheduled) {
                drainScheduled = true
        ...
        if (shouldPostDrain) {
            callbackHandler.post(::drainLatestFrame)
        }
    }

    private fun drainLatestFrame() {
        while (true) {
            val framesToDrain = synchronized(frameSignalSync) {
                if (released) {
                ...
                if (pendingFrameSignals <= 0) {
                    drainScheduled = false
                    return
                }

                val pending = pendingFrameSignals
                pendingFrameSignals = 0
                pending
            }

            drainFrame(framesToDrain.coerceAtLeast(1))
        }
    }

    private fun drainFrame(framesToDrain: Int) {
        makeCurrent(baseEglSurface)
        val texture = surfaceTexture ?: return
        var timestampNs = 0L
        repeat(framesToDrain) {
            texture.updateTexImage()
            texture.getTransformMatrix(texMatrix)
            timestampNs = texture.timestamp
        }

        val shouldRenderBase =
            targetBaseFrameIntervalNs <= 0L ||
                lastBaseRenderTimestampNs == 0L ||
                timestampNs <= 0L ||
                timestampNs - lastBaseRenderTimestampNs >= targetBaseFrameIntervalNs
        if (shouldRenderBase) {
            renderBase(timestampNs)
            lastBaseRenderTimestampNs = timestampNs
        }

        val roiSnapshot = roiProvider?.invoke()
        if (roiSnapshot?.isActive == true && enhancementEglSurface !== EGL14.EGL_NO_SURFACE) {
            renderEnhancement(timestampNs, roiSnapshot)
            onEnhancementRendered?.invoke(timestampNs / 1_000L, roiSnapshot)
        }
    }

    override fun close() {
        runOnCallbackThreadBlocking {
            closeInternal()
        }
    }
    ...
    private fun runOnCallbackThreadBlocking(action: () -> Unit) {
        if (Looper.myLooper() == callbackHandler.looper) {
            action()
            return
        }
    ...
    private fun initializeGl() {
        ...
        enhancementEglSurface = enhancementInputSurface?.let { createWindowSurface(config, it) } ?: EGL14.EGL_NO_SURFACE
        ...
        surfaceTexture = SurfaceTexture(externalTextureId).apply {
            ...
            setOnFrameAvailableListener(this@SplitStreamGlHub)
        }
