...
internal sealed class MediaFoundationD3D11PlaybackController : IPlaybackController
{
    ...
    private long _lastPresentedBaseSourcePresentationTimeUs;
    ...
    private bool _enhancementPathAvailable;
    ...
    public long LastPresentedBasePresentationTimeUs => Interlocked.Read(ref _lastPresentedBaseSourcePresentationTimeUs);
    ...
    public void UpdateAggressiveMode(bool enabled)
    {
        ...
        ReceiverTrace.Log($"MF UpdateAggressiveMode: {_aggressiveMode} -> {enabled}");
    ...
    public void UpdateUltraLowLatencyMode(bool enabled)
    {
        ...
        ReceiverTrace.Log($"MF UpdateUltraLowLatencyMode: {_ultraLowLatencyMode} -> {enabled}");
        _ultraLowLatencyMode = enabled;
        _queue?.UpdateJitterBufferDelay(_adaptiveJitterDelay);
        _enhancementQueue?.UpdateJitterBufferDelay(TimeSpan.FromMilliseconds(Math.Max(0, _adaptiveJitterDelay.TotalMilliseconds * 0.5)));
    }
    ...
    {
        _manualMinPacingDelay = minDelay > TimeSpan.Zero ? minDelay : TimeSpan.Zero;
        _manualMaxPacingDelay = maxDelay > TimeSpan.Zero ? maxDelay : TimeSpan.Zero;
    }
    ...
    public void ApplySessionConfig(SessionConfig config)
    {
        ...
        Interlocked.Exchange(ref _lastPresentedBaseSourcePresentationTimeUs, 0);
        _enhancementSampleTimeHns = 0;
        _enhancementPresentationBaseUs = null;
        _lastEnhancementNormalizedTimeHns = 0;
        _enhancementPathAvailable = false;
    ...
    {
        if (!(_sessionConfig?.IsSplitStream ?? false) || !_enhancementPathAvailable || _enhancementQueue is null || metadata is null)
        {
            return;
        }
    ...
    {
        ReceiverTrace.Log("MF PrepareForSessionStop begin");
        PrepareForKeyFrameRecovery();
        ReceiverTrace.Log("MF PrepareForSessionStop end");
    }
    ...
    private void RestartPlayback()
    {
        ...
        var restartStartedAt = Stopwatch.GetTimestamp();
        ReceiverTrace.Log($"MF RestartPlayback begin: {_sessionConfig.Codec} {_sessionConfig.Width}x{_sessionConfig.Height} split={_sessionConfig.IsSplitStream} ultra={_ultraLowLatencyMode} aggressive={_aggressiveMode}");
        StopWorker();
        _queue?.Dispose();
        ...
        var cinemaSmooth = _sessionConfig.IsCinemaSmooth;
        var lowLatencyDecodeProfile =
            (!cinemaSmooth && _ultraLowLatencyMode) ||
            _aggressiveMode ||
        ...
        var jitterBufferDelay = !isAdbTunnel && (_ultraLowLatencyMode || cinemaSmooth)
            ? _adaptiveJitterDelay
            : TimeSpan.Zero;

        _queue = new LatestAccessUnitQueue(
            maxQueuedUnits: cinemaSmooth ? 6 : (lowLatencyDecodeProfile ? (allowUdpHeadroom ? 2 : 1) : 2),
            maxQueuedBytes: isAdbTunnel
                ? (cinemaSmooth ? 1024 * 1024 : (_ultraLowLatencyMode ? 384 * 1024 : (lowLatencyDecodeProfile ? 512 * 1024 : 768 * 1024)))
                : (cinemaSmooth ? 2 * 1024 * 1024 : (_ultraLowLatencyMode ? (allowUdpHeadroom ? 640 * 1024 : 512 * 1024) : (lowLatencyDecodeProfile ? 768 * 1024 : 1024 * 1024))),
            statsChanged: stats => StreamStatsChanged?.Invoke(stats),
            hardResetOnKeyFrame: isAdbTunnel && !cinemaSmooth,
            dropCurrentOnWaitForKeyFrame: !cinemaSmooth,
            preferLatestQueuedUnit: !cinemaSmooth && !isAdbTunnel && _ultraLowLatencyMode,
        ...
        if (_sessionConfig.IsSplitStream)
        {
            try
            {
                _enhancementQueue = new LatestAccessUnitQueue(
                    maxQueuedUnits: cinemaSmooth ? 2 : 1,
                    maxQueuedBytes: cinemaSmooth ? 512 * 1024 : 384 * 1024,
                    statsChanged: stats => EnhancementStreamStatsChanged?.Invoke(stats),
                    hardResetOnKeyFrame: !cinemaSmooth,
                    dropCurrentOnWaitForKeyFrame: !cinemaSmooth,
                    preferLatestQueuedUnit: !cinemaSmooth,
                    jitterBufferDelay: cinemaSmooth ? TimeSpan.FromMilliseconds(Math.Max(0, _adaptiveJitterDelay.TotalMilliseconds * 0.5)) : TimeSpan.Zero);
                ...
                _enhancementPathAvailable = true;
            }
            catch
            {
                DisableEnhancementPath();
            }
        }
        UpdateStatus(_softwareSafeMode ? "Opening (MF software-safe)" : "Opening");
        _renderer.Clear();
        ReceiverTrace.Log($"MF RestartPlayback end ({Stopwatch.GetElapsedTime(restartStartedAt).TotalMilliseconds:0.0} ms)");
    }
    ...
    private void StopWorker()
    {
        ...
        ReceiverTrace.Log("MF StopWorker begin");
        var cts = _decodeCts;
        ...
        ReceiverTrace.Log($"MF StopWorker end ({Stopwatch.GetElapsedTime(stopStartedAt).TotalMilliseconds:0.0} ms)");
    }

    private void DisableEnhancementPath()
    {
        _enhancementDecodeCts?.Cancel();
        _enhancementDecodeTask = null;
        _enhancementDecodeCts?.Dispose();
        _enhancementDecodeCts = null;
        _enhancementDecoderSession?.Dispose();
        _enhancementDecoderSession = null;
        _enhancementQueue?.Dispose();
        _enhancementQueue = null;
        _enhancementPathAvailable = false;
        lock (_enhancementMetadataByPts)
        {
            _enhancementMetadataByPts.Clear();
        }
        EnhancementStreamStatsChanged?.Invoke(new PlaybackStreamStats
        {
            QueuedAccessUnits = 0,
            QueuedBytes = 0,
    ...
    private void OnBaseFrameRendered(long sampleTimeHns)
    {
        if (_basePresentationTimeUs is null)
        {
            Interlocked.Exchange(ref _lastPresentedBaseSourcePresentationTimeUs, 0);
            return;
        }

        var sourcePresentationTimeUs = _basePresentationTimeUs.Value + Math.Max(0, sampleTimeHns / 10L);
        Interlocked.Exchange(ref _lastPresentedBaseSourcePresentationTimeUs, sourcePresentationTimeUs);
    }
    ...
    {
        var cinemaSmooth = _sessionConfig?.IsCinemaSmooth ?? false;
        if ((!_ultraLowLatencyMode && !cinemaSmooth) || sampleTimeHns <= 0)
        {
            return;
        }
        ...
        var maxPacingMs = _manualMaxPacingDelay > TimeSpan.Zero
            ? _manualMaxPacingDelay.TotalMilliseconds
            : (cinemaSmooth ? 14.0 : 4.0);
        var minPacingMs = _manualMinPacingDelay > TimeSpan.Zero
            ? _manualMinPacingDelay.TotalMilliseconds
            : (cinemaSmooth ? 0.50 : 0.15);
        if (maxPacingMs < minPacingMs)
        {
            maxPacingMs = minPacingMs;
        }
        if (waitMs <= minPacingMs || waitMs > maxPacingMs)
        {
            return;
        }
    ...
    private sealed class MediaFoundationDecoderSession : IDisposable
    {
        ...
        private readonly Action<long> _onBaseFrameRendered;
        ...
        public MediaFoundationDecoderSession(
            ...
            Action<long> onBaseFrameRendered)
        {
            _renderer = renderer;
            _config = config;
            _onBaseFrameRendered = onBaseFrameRendered;
        ...
        private IMFTransform CreateDecoder(SessionConfig config)
        {
            var inputType = new RegisterTypeInfo
            {
                ...
                GuidSubtype = config.Codec.Contains("av1", StringComparison.OrdinalIgnoreCase)
                    ? new Guid("31305641-0000-0010-8000-00AA00389B71")
        ...
        private void ConfigureInputType(SessionConfig config)
        {
            ...
            inputType.Set(
                ...
                config.Codec.Contains("av1", StringComparison.OrdinalIgnoreCase)
                    ? new Guid("31305641-0000-0010-8000-00AA00389B71")
                    : config.Codec.Contains("hevc", StringComparison.OrdinalIgnoreCase)
                        ? VideoFormatGuids.HevcEs
        ...
        private bool TryRenderDxgiSample(IMFSample sample)
        {
            try
            {
                ...
                _onBaseFrameRendered(sample.SampleTime);
    ...
    private sealed class MediaFoundationEnhancementDecoderSession : IDisposable
    {
        ...
        private IMFTransform CreateDecoder()
        {
            ...
            {
                GuidMajorType = MediaTypeGuids.Video,
                GuidSubtype = codecMime.Contains("av1", StringComparison.OrdinalIgnoreCase)
                    ? new Guid("31305641-0000-0010-8000-00AA00389B71")
                    : codecMime.Contains("hevc", StringComparison.OrdinalIgnoreCase)
                        ? VideoFormatGuids.HevcEs
                        : VideoFormatGuids.H264Es,
            };
        ...
        private void ConfigureInputType()
        {
            ...
            inputType.Set(
                MediaTypeAttributeKeys.Subtype,
                (_config.EnhancementCodec ?? _config.Codec).Contains("av1", StringComparison.OrdinalIgnoreCase)
                    ? new Guid("31305641-0000-0010-8000-00AA00389B71")
