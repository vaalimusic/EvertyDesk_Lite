...
internal sealed class D3D11SwapChainRenderer : IDisposable
{
    ...
    private int _surfaceReadySyncPending;
    ...
    private void EnsureSurfaceReady()
    {
        ...
        if (_playbackHost.IsHandleCreated && _playbackHost.InvokeRequired)
        {
            if (Interlocked.CompareExchange(ref _surfaceReadySyncPending, 1, 0) != 0)
            {
                return;
            }
            ...
            try
            {
                _playbackHost.BeginInvoke(new Action(() =>
                {
                    try
                    {
                        EnsureSurfaceReadyCore();
                    }
                    finally
                    {
                        Interlocked.Exchange(ref _surfaceReadySyncPending, 0);
                    }
                }));
            }
            catch (ObjectDisposedException)
    ...
    private void EnsureSurfaceReadyCore()
    {
        if (_playbackHost.IsDisposed || _surface.IsDisposed)
        {
            return;
